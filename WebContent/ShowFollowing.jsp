		...
		<div id="midmid">
			...
			<div id="showmenubar">
				...
				<%
					...
					{
				%>
				<div id="menuitem5" class="activehover">
					<div id="following"><center><a class="link" id="activelink" href="Profile?action=following&searchId=<%=userId %>" style="cursor: pointer;">Following</a></center></div>
					<div id="counter1" style="color:#2b7bb9;"><center><label class="ffff"><%=userInfo.getInt("followed") %></label></center></div>
				</div>
				<%
					}
					else
			...
			<%
			getFollowingDAO gfd=new getFollowingDAO();
			ResultSet following = (ResultSet)gfd.getFollowing(userID,userId, con);
			...
			<div id="nostorydiv">
				<div id="displaytag">
					<center>This user is not following anyone.</center>
				</div>
			</div>
			<%
			}
			...
			{
				following.beforeFirst();
			int counter=0;
			while(following.next())
			{
				counter++;
				
				if(following.getObject("lable") == null)
					lable="Follow";
				else
					lable="Following";
				
				if(following.getObject("bio") == null)
					bio=" ";
				else
					bio=following.getString("bio");
			%>
			
			<div class="followerbox" id="fbox<%=counter %>">
				<div id="followerpic">
					<img  src="<%=request.getContextPath() %>/uploadedImages/<%=following.getString("profile_pic") %>" height="50" width="50" style=" float:left" />
				</div>
				<div id="followernameandid">
					<div id="followername"><a class="a1" href="Profile?action=profilebook&searchId=<%=following.getString("uid") %>" ><%=following.getString("fullname") %></a></div>
					<div id="followerid"><%=following.getString("uid") %></div>
				</div>
				<div id="followerdetails">
					<div id="messagearea">
						<div id="follower-messages">MESSAGES</div>
						<div id="follower-counter"><%=following.getString("messages") %></div>
					</div>
					<div id="followersarea">
						<div id="follower-followers">FOLLOWER</div>
						<div id="follower-counter"><%=following.getString("followers") %></div>
					</div>
					<div id="followingarea">
						<div id="follower-following">FOLLOWING</div>
						<div id="follower-counter"><%=following.getString("followed") %></div>
					</div>
				</div>
				<div id="followerbio">
				...
				<%
				if(Integer.parseInt(following.getString("uid")) != userId)
				{
				%>
				<div id="followbutton1">
					<form id="form1" name=form1>
						<input type="hidden" name="follower" value="<%=userId %>" id="s<%=counter %>"/>
						<input type="hidden" name="followed" value="<%=following.getString("uid") %>" id="r<%=counter %>"/>
