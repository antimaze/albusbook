...
<title>Albusbook / Who To Follow</title>
		...
		<div id="midmid">
			<div id="showmenubar">
				<div id="messagebooksignarea">
					<label>/ People who to follow</label>
				</div>
			</div>
			
			<%
			
			ResultSet whoToFollow =(ResultSet) sd.getAllWhoToFollow(userId,con);
			String bio = null;
			if(!whoToFollow.next())
			{
			%>
			<div id="nostorydiv">
				<div id="displaytag">
					<center>There are no people we can suggest to follow.</center>
				</div>
			</div>
			<%
			}
			...
			while(whoToFollow.next())
			{
				...
				if(whoToFollow.getObject("bio") == null)
					bio=" ";
				else
					bio=whoToFollow.getString("bio");
			...
			<div class="followerbox" id="fbox<%=counter %>">
				<div id="followerpic">
					<img  src="<%=request.getContextPath() %>/uploadedImages/<%=whoToFollow.getString("profile_pic") %>" height="50" width="50" style=" float:left" />
				</div>
				<div id="followernameandid">
					<div id="followername"><a class="a1" href="Profile?action=profilebook&searchId=<%=whoToFollow.getString("uid") %>" ><%=whoToFollow.getString("fullname") %></a></div>
					<div id="followerid"><%=whoToFollow.getString("uid") %></div>
				</div>
				<div id="followerdetails">
					<div id="messagearea">
						<div id="follower-messages">MESSAGES</div>
						<div id="follower-counter"><%=whoToFollow.getString("messages") %></div>
					</div>
					<div id="followersarea">
						<div id="follower-followers">FOLLOWERS</div>
						<div id="follower-counter"><%=whoToFollow.getString("followers") %></div>
					</div>
					<div id="followingarea">
						<div id="follower-following">FOLLOWING</div>
						<div id="follower-counter"><%=whoToFollow.getString("followed") %></div>
					</div>
				</div>
				<div id="followerbio">
				...
				<div id="followbutton1">
					<form id="form1" name=form1>
						...
						<input type="button" name="follow1<%=counter %>" id="follow1<%=counter %>" class="history1" value="Follow" onclick="f2(<%=counter %>)"  >
					</form>
				</div>
			</div>
			<%
